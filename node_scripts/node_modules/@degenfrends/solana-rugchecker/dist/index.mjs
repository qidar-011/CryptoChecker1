...
// src/checker/metadata-checker.ts
var validator = __require("validator");
config();
  ...
  getHeliusMetadata(tokenAddress, token) {
    return __async(this, null, function* () {
      ...
      try {
        ...
        if (jsonUri) {
          const metadataResponse = yield axios.get(jsonUri, {
            timeout: 3e5,
            responseType: "json"
          });
...
var _HoldersChecker = class _HoldersChecker {
  constructor({ solanaRpcEndpoint }, connection) {
    ...
    if (!connection) {
      connection = new Connection2(solanaRpcEndpoint);
    }
    this.connection = connection;
  }
  check(tokenAddress) {
    return __async(this, null, function* () {
      var _a, _b, _c, _d, _e;
      const mintAddress = new PublicKey2(tokenAddress);
...
var _LiquidityChecker = class _LiquidityChecker {
  constructor({ solanaRpcEndpoint, poolFilePath, poolAddress }, connection) {
    ...
    if (!connection) {
      connection = new Connection3(solanaRpcEndpoint);
    }
    this.connection = connection;
  }
  check(tokenAddress) {
    return __async(this, null, function* () {
      ...
      const accInfo = yield this.connection.getParsedAccountInfo(new PublicKey3(lpMint));
  ...
  getLiquidityPool(tokenAddress) {
    return __async(this, null, function* () {
      try {
        const data = yield fs.readFile(this.poolFilePath, "utf8");
...
var whois = __require("whois-json");
...
var _MarketdataChecker = class _MarketdataChecker {
  ...
  check(tokenAddress) {
    return __async(this, null, function* () {
      const marketdataResponse = yield axios3.get("https://api.dexscreener.com/latest/dex/tokens/" + tokenAddress, {
        timeout: 3e5,
        responseType: "json"
      });
